...
contract VaultOwned is Ownable {
    ...
    mapping(address => bool) private _vault;
     ...
     */
    modifier onlyVault() {
        require( _vault[msg.sender] == true, "VaultOwned: caller is not the Vault" );
        _;
    }
...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "CrusaderERC20TOken: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "CrusaderERC20TOken: TWAP source not present." );
...
contract TycheERC20Token is Divine {

    using SafeMath for uint256;
    using Address for address;

    uint256 public minReferAmount;
    uint256 public minReferHoldingAmount;
    address private teamAddress;

    mapping (address => address) private referrer;

    constructor() Divine("Tyche", "THE", 18) {
//        mint(teamWallet, _amount);
    }
   ...
   */
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        _transfer(msg.sender, recipient, amount);
        _bindRefer(msg.sender, recipient, amount);
        return true;
    }
    ...
    function _bindRefer(address sender, address recipient, uint256 amount) internal {
        if (_isContractTransaction(sender, recipient) == true){
            return;
        }
        if (amount < minReferAmount){
            return;
        }
        if(balanceOf(sender) < minReferHoldingAmount){
        	return;
        }
        if (referrer[recipient] != address(0)) {
            return;
        }
    ...
    function setMinReferHoldingAmount(uint256 amount) external onlyOwner(){
		minReferHoldingAmount = amount;
	}

    function getReferrerAddr(address addr) view public returns(address) {
    	address _referrer = referrer[addr];
        if(balanceOf(_referrer) < minReferHoldingAmount){
            _referrer = address(0);
        }
    	return _referrer;
    }
